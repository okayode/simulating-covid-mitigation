...
row_r=df0['Country_Region'].tolist().index('Korea, South')
total_recov = [df0[day.strftime('%-m/%-d/%y')][row_r] for day in days]

row_c=df1['Country_Region'].tolist().index('Korea, South')
total_cases = [df1[day.strftime('%-m/%-d/%y')][row_c] for day in days]

t = np.reshape(dd, [-1])
R = np.reshape(total_recov, [-1])
new_R = R*100/(51.64*10**6) # rescale y-axis

I = np.reshape(total_cases, [-1])
new_I = I*100/(51.64*10**6) # rescale y-axis
...
class PINN_constantParam:
    ...
    def __init__(self, t, I, R, beta, layers, lb, ub):
...
model1 = PINN_constantParam(t_train, I_train, R_train, 0.10, layers, lb, ub)
model2 = PINN_constantParam(t_train, I_train, R_train, 0.05, layers, lb, ub)
model3 = PINN_constantParam(t_train, I_train, R_train, 0.025, layers, lb, ub)
model4 = PINN_constantParam(t_train, I_train, R_train, 0.01, layers, lb, ub)
...
gamma_1 = model1.sess.run(model1.gamma)
gamma_2 = model2.sess.run(model2.gamma)
gamma_3 = model3.sess.run(model3.gamma)
gamma_4 = model4.sess.run(model4.gamma)
...
print("FirstGamma:",*["%.8f"%(x) for x in gamma_1])
print("SecondGamma:",*["%.8f"%(x) for x in gamma_2])
print("ThirdGamma:",*["%.8f"%(x) for x in gamma_3])
print("FourthGamma:",*["%.8f"%(x) for x in gamma_4])
